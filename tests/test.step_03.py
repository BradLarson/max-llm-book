...
def test_step_03():
    ...
    if 'self.weight = Tensor.ones([dim])' in source or 'self.weight = Tensor.ones([' in source:
    ...
    if 'self.bias = Tensor.zeros([dim])' in source or 'self.bias = Tensor.zeros([' in source:
    ...
    if 'gamma=self.weight' in source and 'beta=self.bias' in source and 'epsilon=self.eps' in source:
        ...
        print("   Hint: Use F.layer_norm(x, gamma=self.weight, beta=self.bias, epsilon=self.eps)")

    # Test 8: Check that None values are replaced
    lines = source.split('\n')
    none_assignments = [line for line in lines if ('self.weight = None' in line or
                                                     'self.bias = None' in line or
                                                     'return None' in line)]
    ...
    try:
        ...
        if hasattr(layer_norm, 'eps'):
            if layer_norm.eps == 1e-5:
                ...
                print(f"❌ LayerNorm.eps has incorrect value: expected 1e-05, got {layer_norm.eps}")
        else:
        ...
        print("⚠️  Cannot test layer normalization execution (MAX not available in this environment)")
